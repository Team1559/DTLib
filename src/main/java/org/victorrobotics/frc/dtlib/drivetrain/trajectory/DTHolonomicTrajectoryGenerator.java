...
import org.victorrobotics.frc.dtlib.drivetrain.trajectory.DTHolonomicTrajectory.Point;
import org.victorrobotics.frc.dtlib.math.geometry.DTVector2DR;
import org.victorrobotics.frc.dtlib.math.spline.DTLinearSpline;
import org.victorrobotics.frc.dtlib.math.spline.DTSpline;
...
public class DTHolonomicTrajectoryGenerator {
    ...
    private void applyCentripetalConstraint(Point[] points) {
        ...
        for (int i = 1; i < points.length - 1; i++) {
            points[i].curvatureRadius = computeRadius(points[i - 1].position, points[i].position,
                    points[i + 1].position);
    ...
    private void applyDecelerationConstraint(Point[] points) {
        ...
        for (int i = maxIndex - 1; i >= 0; i--) {
            ...
            double maxVel = Math.sqrt(
                    nextVel * nextVel + 2 * nextMaxAccel * (points[i + 1].distance - points[i].distance));
